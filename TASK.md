...
3\. [DONE — completed 2026-06-19] Fix hardcoded `PROJECT_ROOT`. `app.py` and `ask_aruvi_agent.py` were fixed in May 2026, but `ask_aruvi_qa.py` and `ask_aruvi_feedback.py` (in both `aruvi_streamlit/` and `aruvi-scripts/`) were still hardcoded — found during the 2026-06-19 readiness review and fixed. All runtime modules now use `Path(__file__).resolve().parent.parent`. Verified: `grep -rn /Users/kumar_radhakrishnan --include=*.py` returns nothing.
...
6\. [DONE — 2026-06-19] Readiness review (Part B local hardening): created `requirements.txt` (pinned runtime deps; svglib confirmed NOT required); untracked 287 textbook PDFs (~1.5 GB) from git via `git rm --cached` + `.gitignore` (files remain on disk); cleaned duplicate `.gitignore` lines. API-key rotation handled separately by team. **Porting prep:** relocated the 3 runtime files (`token_log.csv`, `ask_aruvi.csv`, `api_rates.json`) out of `knowledge_commons/evaluation_mappings/` into new top-level `runtime_data/` (via `git mv`), updated paths in `app.py` (×3) + `aruvi_config.json` (`token_log` key), and added `parent.mkdir` guards to both log writers so logging survives a fresh deploy. Archival eval-report DOCX stay in `knowledge_commons/`. Result: the runtime app has ZERO dependency inside `knowledge_commons/` — porting "everything except knowledge_commons" (+ `runtime_data/`, `miscellaneous/`, `aruvi_config.json`, `requirements.txt`) is now clean. Porting boundary documented in CLAUDE.md §7.

**App-folder restructure (2026-06-19):** moved the entire runtime bundle under a single top-level `app/` folder — `app/{aruvi_streamlit, mirror, miscellaneous, runtime_data, lpa_page.html, allocate_page.html, requirements.txt}`. This folder is now the ONLY thing deployed to the cloud. Runtime app needed ZERO code changes (`PROJECT_ROOT` auto-derives to `app/`; all reads are `PROJECT_ROOT / "..."`). Authoring side updated to match: `aruvi_config.json` `mirror*`/`token_log` keys prefixed with `app/`, and hardcoded `mirror` refs in `aruvi-scripts/*.py` (refresh_kb, ask_aruvi_qa/feedback, gpt_lpa_test) repointed to `app/mirror`. `run_mapping.sh` needed no change (passes root-level `aruvi_config.json`). Verified: app compiles in new location, all runtime paths resolve under `app/`, authoring config resolves `mirror`→`app/mirror`, and `grep knowledge_commons` under `app/aruvi_streamlit/` returns nothing. Two corrections found en route: the two root HTML files are runtime templates (not debug output) and `aruvi_config.json` is authoring-only (runtime app never reads it).

7\. [DEFERRED — cloud-era levers, documented in CLAUDE.md §7] (a) flip `USE_PROMPT_CACHE=True` when volume rises / chapter re-runs within the 1h TTL become common; (b) per-user/tenant isolation for saved_plans + feedback + token_log (move to DB; mirror inputs to shared/object storage); (c) background generation threads → task queue when running multiple instances; (d) add Anthropic API retry/error handling. No action until cloud path is chosen.
...
| English | 8 | 8/8 ✅ |
| Science | 13 | 0 ❌ |
| Mathematics | 8 | 6/8 ✅ (ch_01, ch_02, ch_03, ch_04, ch_06 mapped; ch_05 summary only) |
| Social Sciences | 0 PDFs loaded | 0 ❌ |

**Next steps for secondary stage:**
- Science IX: 13 chapters ready to run (textbook PDFs present)
- Mathematics IX: 8 chapters ready to run (textbook PDFs present) — pipeline fully wired (2026-06-16). Chapter 7 ("The Mathematics of Maybe: Introduction to Probability") run 2026-06-18 via explicit prompt paths: `ch_07_summary.json` + `ch_07_mapping.json` (core_cg CG-6, core C-6.2, adjunct C-8.1, co_central false, EI 7.5). Chapter 1 ("Orienting Yourself: The Use of Coordinates") run 2026-06-19 via explicit prompt paths: `ch_01_summary.json` + `ch_01_mapping.json` (core_cg CG-4, core C-4.5, adjunct C-8.1, co_central false, EI 7.5). Chapter 3 ("The World of Numbers") run 2026-06-19 via explicit prompt paths: `ch_03_summary.json` + `ch_03_mapping.json` (core_cg CG-1, core C-1.1, adjunct C-7.1/C-9.1, co_central false, EI 15.0 — highest effort index of the four chapters run so far, driven by the formal proof that √2 is irrational and the full number-set hierarchy). Chapter 6 ("Measuring Space: Perimeter and Area") run 2026-06-19 via explicit prompt paths: `ch_06_summary.json` + `ch_06_mapping.json` (core_cg CG-5, core C-5.1, adjunct C-7.1/C-9.1, co_central false, EI 11.0). Note: ch_02 and ch_04 were found already complete (summary+mapping) on disk during this session — this table previously undercounted them; they were run in an earlier, undocumented session. ch_05 has a summary but no mapping yet. Remaining work: run mapping for ch_05, and run summary+mapping for ch_08. The `chapter` skill's SKILL.md still needs a manual update (paste `chapter_skill_updated.md` from project root via Settings > Capabilities) before the skill will route Mathematics secondary requests to the correct prompt folder; until that paste happens, request the secondary mathematics prompts by their explicit path.
