...
- **Provider seam + certified model IDs**: All runtime model calls route through `aruvi_streamlit/llm_client.py` — a thin passthrough (`stream()` / `create()`) over the Anthropic SDK. The three runtime call sites (two `app.py` LP/assessment generators + `ask_aruvi_qa.py`) use it; never instantiate `anthropic.Anthropic()` or call `client.messages.*` directly in app code. The certified model IDs live there too: `GENERATION_MODEL` (Sonnet, LP/assessment) and `HELPLINE_MODEL` (Haiku, Ask Aruvi). Aruvi is built and certified **Claude-only** — a Claude version bump is a one-line change here, but requires re-running a regression cycle first. The in-house authoring scripts (`call_mapping_api.py`, `refresh_kb.py`) deliberately do NOT use the seam — customers never run them.  
- **Hardcoded paths — RESOLVED (2026-06-19)**: All runtime modules now derive `PROJECT_ROOT` from `Path(__file__).resolve().parent.parent` — `app.py`, `ask_aruvi_agent.py`, and (fixed this session) `ask_aruvi_qa.py` + `ask_aruvi_feedback.py` in both `aruvi_streamlit/` and the duplicate copies in `aruvi-scripts/`. The app is now machine-portable. Carry-forward: grep `/Users/kumar_radhakrishnan` across `*.py` before any release — it must return nothing.
