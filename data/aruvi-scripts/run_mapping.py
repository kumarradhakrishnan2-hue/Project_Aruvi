...
def run_single_chapter(chapter_pdf: str, chapter_number: int, paths: dict, skill_dir: str):
    ...
    print("  [1/3] Extracting chapter text...")
    ...
    print("  [2/3] Loading Curricular Goals...")
...
def main():
    ...
    if token_log.exists():
        ...
        if run_rows:
            ...
            print(f"\nToken totals ({len(run_rows)} calls):")
            print(f"  Input  : {total_in:,} tokens")
            print(f"  Output : {total_out:,} tokens")
            print(f"  Cost   : ₹{total_cost:.2f}")
            print(f"  Per ch : ₹{total_cost/len(run_rows):.2f} average")
