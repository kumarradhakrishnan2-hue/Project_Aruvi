...
[Learning #32] — 2026-06-19 — Mathematics Secondary: Chapter 1 run; no worked-example banners is a valid chapter shape

Context: Second live execution of the Mathematics secondary pipeline (Learning #31 was the first, Ch 7). Ran "chapter skill for chapter 1 grade ix mathematics" — since the `chapter` skill's SKILL.md paste-in is still pending, used the explicit prompt paths directly per TASK.md's standing instruction.

What was run: Step 1 read `knowledge_commons/textbooks/mathematics/ix/chapter 01 - Orienting Yourself-The Use of Coordination.pdf` directly and wrote `mirror/chapters/mathematics/ix/summaries/ch_01_summary.json` — 4 sections (1.1-1.4), 0 worked examples, 13 exercises (End-of-Chapter Q6-Q13 correctly excluded as asterisked), 2 think_reflect notes, effort signals CD:2 RL:1 EL:1. Step 2 wrote `ch_01_mapping.json`: core_cg CG-4, core C-4.5 (coordinate geometry — the chapter's entire organising purpose), adjunct C-8.1 (daily-life modelling via the city-grid and screen-pixel exercises), co_central false, effort_index 7.5 via (2×2)+(1×2)+(1×1.5). Full verification checklist passed.

Observation: This chapter has zero items meeting the "Example N" worked-example banner test — all of its worked content (the room-plan narrative, the point-labelling walkthrough, the distance-formula derivation for triangle ADM) is narrated inline rather than presented as discrete banner-labelled "Example" instances. The step_1 prompt's schema does not require a non-empty `enumerated_worked_examples` array (only `enumerated_exercises` is flagged as "non-empty for a normal chapter"), so an empty worked-examples array for an introductory, narrative-driven chapter is a correct reading of the source, not a missed-enumeration error.

---

[Learning #33] — 2026-06-19 — Mathematics Secondary: Chapter 6 run; TASK.md progress table found undercounting two already-completed chapters

Context: Third live execution of the Mathematics secondary pipeline this week (after Ch 7 in #31, Ch 1 in #32). Ran "chapter skill for chapter 6 grade ix mathematics" — used explicit prompt paths directly since the `chapter` skill's SKILL.md paste-in is still pending. Before starting, a bash inspection of `mirror/chapters/mathematics/ix/summaries/` and `mappings/` (a verify-before-trusting-the-table step, per Learning #13's carry-forward rule) found ch_02 and ch_04 already had complete summary+mapping pairs on disk, and ch_05 had a summary but no mapping — none of this was reflected in TASK.md, which still read "3/8 (ch_01, ch_03, ch_07)". These two chapters (ch_02, ch_04) were evidently run in an undocumented earlier session with no corresponding MEMORY.md entry.

What was run: Step 1 read `knowledge_commons/textbooks/mathematics/ix/chapter 06 - Measuring Space-Perimeter and Area.pdf` directly and wrote `mirror/chapters/mathematics/ix/summaries/ch_06_summary.json` — 10 sections (6.1-6.10), 7 worked examples (Heron's formula on equilateral/isosceles/3-4-5 triangles, Brahmagupta's formula on a cyclic rectangle and an isosceles trapezium), 34 exercises (Exercise 6.3 Q7-Q10 and most of the End-of-Chapter set's higher items correctly excluded as asterisked), 3 think_reflect notes, effort signals CD:2 RL:2 EL:2. Step 2 wrote `ch_06_mapping.json`: core_cg CG-5 (deriving/using area formulae), core C-5.1 (Heron's and Brahmagupta's formulas, named explicitly in the CG document's own competency text), adjunct C-7.1 (the algebraic proof that Brahmagupta's formula reduces to Heron's when one side vanishes) and C-9.1 (decomposing pathway/crossroad/circular-path/composite-figure problems into known shapes), co_central false, effort_index 11.0 via (2×2)+(2×2)+(2×1.5). Full verification checklist passed (CG validity, core/adjunct CG membership, counts ≤2, co_central consistency, dissolution_test names CG-5's operation, effort signals match, formula arithmetic correct).

Observation: Unlike Ch 7 (single clean core, no competing candidate) and Ch 1 (zero worked examples, narrative-driven), Ch 6 is the first secondary Mathematics chapter where a plausible second core competency had to be actively ruled out — the pathway/crossroad/circular-path/composite-figure sections (6.8-6.10) could superficially look like a distinct "measurement application" strand from the triangle/quadrilateral-formula strand (6.2-6.7), but per Rule 2b they are not cognitively distinct: both strands are vehicles for the same C-5.1 area-formula skill, just applied to different shapes. This is the constitution's own worked distinction in Rule 2a ("a section that, read alone, looks like a different CG... is in service of the chapter's actual subject and does NOT redirect the selection") applied at the competency level rather than the CG level.

Action taken: TASK.md's Secondary Stage (IX) Mathematics row corrected from "3/8 (ch_01, ch_03, ch_07)" to "6/8 (ch_01, ch_02, ch_03, ch_04, ch_06 mapped; ch_05 summary only)", with a note flagging the undocumented earlier ch_02/ch_04 run and identifying ch_05 (summary exists, mapping missing) and ch_08 (neither exists) as the two genuinely remaining gaps.

Carry-forward rule: Before reporting "N/8 done" for any chapter-pipeline subject, always bash-`ls` both the summaries/ and mappings/ folders directly rather than trusting the last-written progress table — this is the second time (after Learning #13's data-source-verification context) that a stale table was caught only by direct folder inspection, and the gap here was two full chapters, not a rounding error. If a chapter has a summary but no mapping (like ch_05 here), call that out explicitly rather than rounding it into either the "done" or "not started" bucket.

Carry-forward rule: Do not treat an empty `enumerated_worked_examples` array as a red flag during verification — only `enumerated_exercises` is expected to be non-empty per the step_1 constraints. A chapter that teaches through continuous narrative example (rather than discrete "Example N" banners) legitimately produces zero worked examples. Also reconfirmed: Glob intermittently fails to find files/folders that direct Read/bash calls resolve correctly (same friction as Learning #31) — fall back to `mcp__workspace__bash` (ls/find) when Glob reports "No files found" for a path you have reason to believe exists.
